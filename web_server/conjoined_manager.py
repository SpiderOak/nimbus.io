...
def list_conjoined_archives(connection, collection_id):
    ...
    return connection.fetch_all_rows("""
        select identifier, key from nimbusio_node.conjoined where
...
def start_conjoined_archive(connection, collection_id, key):
    ...
    result = connection.fetch_one_row("""
        select identifier from nimbusio_node.conjoined
    ...
    connection.execute("""
        insert into nimbusio_node.conjoined (collection_id, identifier, key) 
        values (%s, %s, %s);""", 
