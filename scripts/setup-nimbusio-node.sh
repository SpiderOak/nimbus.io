...
do
    ...
    # xxx, needed sudo -u postgres here.
    sudo -u postgres createuser --login  --no-superuser --no-createdb --no-createrole "nimbusio_node_user.${nodename}" 
    sudo -u postgres createdb --owner="nimbusio_node_user.${nodename}" "nimbusio_node.${nodename}"
